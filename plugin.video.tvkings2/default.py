...
#############Defined Strings#############
addon_id     = 'plugin.video.tvkings2'
...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.tvkings2/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.tvkings2/resources/catchup', 'g'))
...
durextvfol = xbmc.translatePath(os.path.join(ADDON_DATA,'plugin.video.tvkings2'))
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("TV Kings", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search TV Kings:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('TV Kings', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('TV Kings', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('TV Kings', 'PVR & iVue Integration Complete')
			xbmc.executebuiltin('ActivateWindow(10025,"plugin://plugin.video.IVUEcreator/update",return)')
			xbmc.executebuiltin("XBMC.ActivateWindow(Home)")
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('TV Kings', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('TV Kings', 'PVR & iVue Integration Complete')
		...
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.tvkings2/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('TV Kings', '[COLOR red]Do Not Skip[/COLOR] Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.tvkings2')
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('TV Kings','Would You like us to Setup the TV Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['iVue TV Guide','PVR TV Guide','Both'])
			if dialog==0:
				ivueint()
				xbmcgui.Dialog().ok('TV Kings', 'iVue Integration Complete')
				xbmc.executebuiltin('ActivateWindow(10025,"plugin://plugin.video.IVUEcreator/update",return)')
				xbmc.executebuiltin("XBMC.ActivateWindow(Home)")
			elif dialog==1:
				pvrsetup()
